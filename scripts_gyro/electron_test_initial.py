...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    1) Initial Idea. The effort applied to the joint will toggle every 100 steps between negative and positive values.
    ...
    effort_target = torch.ones_like(robot.data.applied_torque)
    effort_target[:, 0] = fmag
    effort_target[:, 1] = -fmag
    # joint_limits = robot.data.joint_limits.clone()
    # joint_position_target = torch.tensor([[joint_limits[0, 0].item(), joint_limits[1, 1].item()]])
    count = 0
